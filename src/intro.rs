...
use crate::{
    assets::{
        iff::Image,
        intro::{Assets, TableSet, CGA_FONT},
    },
    config::{Config, ScrollSpeed, TableId},
    sound::player::Player,
...
impl Intro {
    pub fn new(data: &Path, config: Config, table: Option<TableId>) -> Intro {
        ...
        let player = crate::sound::player::play(module, true);
        let (state, text_page) = match table {
            Some(TableId::Table1 | TableId::Table2) => (State::InitDelay(0), 0),
            Some(TableId::Table3 | TableId::Table4) => (State::InitDelay(0), 1),
            None => (State::Slide(0, SlideState::Gap(0)), 0),
        };
        Intro {
            player,
    ...
    fn render_tables(&self, data: &mut [u8], pal: &mut [(u8, u8, u8)], f: impl Fn(usize) -> bool) {
        let (t1, t2) = if self.text_page % 2 == 0 {
    ...
    fn render_options(
        ...
        if self.config.options.hires {
            lines[6][16..20].copy_from_slice(b"HIGH");
        } else {
            lines[6][16..22].copy_from_slice(b"NORMAL");
        }
    ...
    fn next_page(&mut self) {
        self.text_page += 1;
        if self.text_page == self.assets.text_pages.len() {
            self.text_page = 0;
        }
    }
}
        ...
        match self.state {
            State::Slide(ref mut slide_idx, ref mut sstate) => {
                ...
                match sstate {
                    ...
                    SlideState::Show => {
                        if self.player.get_ticks() >= slide.fade_out_tick
                    ...
                    SlideState::FadeOut(ref mut n) => {
                        *n += 1;
                        if *n >= slide.fade_out_frames {
                            *slide_idx += 1;
                            if *slide_idx == self.assets.slides.len() || self.key == KeyPress::Space
                            {
            ...
            State::Options(ref mut cursor) => {
                match self.key {
                    KeyPress::Enter | KeyPress::Space => match *cursor {
                        ...
                        4 => self.config.options.hires = !self.config.options.hires,
